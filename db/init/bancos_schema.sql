...
CREATE INDEX idx_cuentas_bancarias_banco ON bancos.cuentas_bancarias(id_banco);
CREATE INDEX idx_cuentas_bancarias_estado_activa ON bancos.cuentas_bancarias(id_banco)
  WHERE estado='ACTIVA';

-- ===============================
-- Ledger unificado de movimientos
-- ===============================
CREATE TYPE bancos.tipo_mov AS ENUM ('DEPOSITO','RETIRO','TRANSFERENCIA_IN','TRANSFERENCIA_OUT','CHEQUE_EMITIDO','CHEQUE_COBRADO');

CREATE TABLE bancos.movimientos_bancarios (
...
CREATE INDEX idx_movs_cuenta_fecha ON bancos.movimientos_bancarios(id_cuenta_bancaria, fecha DESC);
CREATE INDEX idx_movs_ref_externa   ON bancos.movimientos_bancarios(referencia_externa);
CREATE INDEX idx_movs_transfer_id   ON bancos.movimientos_bancarios(transferencia_id);

-- ===============================
-- Cheques (opcional, vinculado al ledger)
-- ===============================
CREATE TABLE bancos.tipos_cheque (
...
CREATE INDEX idx_cheques_cuenta ON bancos.cheques(id_cuenta_bancaria);

-- ===============================
-- Conciliaciones
-- ===============================
CREATE TABLE bancos.conciliaciones_bancarias (
...
CREATE INDEX idx_conciliacion_cuenta_fecha ON bancos.conciliaciones_bancarias(id_cuenta_bancaria, fecha_conciliacion DESC);

-- ===============================
-- Proveedores y CxP (intermedio Compras)
-- ===============================
CREATE TABLE bancos.proveedores (
...
CREATE TYPE bancos.estado_factura AS ENUM ('PENDIENTE','PARCIAL','PAGADA','ANULADA');

CREATE TABLE bancos.facturas_compra (
...
CREATE INDEX idx_facturas_prov_estado ON bancos.facturas_compra(proveedor_id, estado);
CREATE INDEX idx_facturas_estado     ON bancos.facturas_compra(estado);
CREATE INDEX idx_facturas_prov_venc  ON bancos.facturas_compra(proveedor_id, fecha_vencimiento);
CREATE TYPE bancos.forma_pago AS ENUM ('TRANSFERENCIA','DEPOSITO','CHEQUE');

CREATE TABLE bancos.pagos_proveedor (
...
CREATE INDEX idx_pagos_prov      ON bancos.pagos_proveedor(proveedor_id, fecha_pago DESC);
CREATE INDEX idx_pagos_factura   ON bancos.pagos_proveedor(factura_id);
CREATE INDEX idx_pagos_cuenta    ON bancos.pagos_proveedor(id_cuenta_bancaria);

-- consultas por moneda o cuenta activa
CREATE INDEX idx_cuentas_moneda   ON bancos.cuentas_bancarias(id_tipo_moneda);
CREATE INDEX idx_facturas_estado  ON bancos.facturas_compra(estado);
CREATE INDEX idx_facturas_prov_venc ON bancos.facturas_compra(proveedor_id, fecha_vencimiento);
-- pendientes (parcial/pendiente) para dashboard
CREATE INDEX idx_facturas_pendientes ON bancos.facturas_compra(estado) WHERE estado IN ('PENDIENTE','PARCIAL');
